...
ORAM::ORAM(int number_of_blocks, string data_location)
    : conf(Config(number_of_blocks)), original_data_ram(data_location, this->conf),
      ...
      stash_reals_count(0), read_count(0), not_found(0)
{
    ...
    {
        tables.emplace_back(Config(current_number_of_blocks));
...
void ORAM ::initialBuild(const std::string &data_location)
{
    ...
    // for(Block block : original_data_ram.memory)
    // {
    //     if(block.state == 1)
    //     {
    ...
    // for(Block block : final_table.data_ram.memory)
    // {
    //     if(block.state == 1)
    //     {
    //         printf("key: %zu, data: %d\n", block.key, block.data);
...
Block generateRandomBlock(uint8_t state = 3)
{
...
std:
    pair<vector<size_t>, vector<size_t>> chunks;
    chunks.first.push_back(0);
    chunks.second.push_back(conf.MU);
    hash_table_one.bins_ram.writeChunks(chunks.first.data(), chunks.second.data(), tmp_blocks, chunks.first.size());
}
...
void ORAM::intersperseStashAndLevelOne()
{
    ...
    hash_table_one.bins_ram.writeChunks(chunks.first.data(), chunks.second.data(), blocks_in_stash, chunks.first.size());
